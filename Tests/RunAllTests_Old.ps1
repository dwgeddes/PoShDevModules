...
if (-not $Quick -and -not $SkipSelfInstall -and ($TestType -eq "All" -or $TestType -eq "SelfInstall")) {
    ...
    # Legacy self-install integration tests (if they exist)
    Invoke-TestSuite "Self-Installation Integration Tests" {
        $selfInstallTestScript = Join-Path $TestsPath "SelfInstallIntegration.Tests.ps1"
        if (Test-Path $selfInstallTestScript) {
            $params = @{}
            if ($Quick) { $params.Quick = $true }
            & $selfInstallTestScript @params
        } else {
            Write-Host "SelfInstallIntegration.Tests.ps1 not found - skipping" -ForegroundColor Yellow
        }
    }
    
    if ($PesterAvailable) {
        Invoke-TestSuite "Self-Installation Unit Tests (Pester)" {
            $selfInstallUnitTests = Join-Path $TestsPath "SelfInstall.Tests.ps1"
            if (Test-Path $selfInstallUnitTests) {
                $pesterConfig = New-PesterConfiguration
                $pesterConfig.Run.Path = $selfInstallUnitTests
                $pesterConfig.Run.PassThru = $true
                $pesterConfig.Output.Verbosity = if ($Quick) { 'Minimal' } else { 'Normal' }
                
                $result = Invoke-Pester -Configuration $pesterConfig
                if ($result.Failed -gt 0) {
                    throw "Self-install unit tests failed: $($result.Failed) failures"
                }
            } else {
                Write-Host "SelfInstall.Tests.ps1 not found - skipping" -ForegroundColor Yellow
            }
        }
    }
}
...
if (($TestType -eq "All" -or $TestType -eq "Practical")) {
    Invoke-TestSuite "Practical Execution Tests" {
        $practicalTestScript = Join-Path $TestsPath "PracticalExecutionTests.ps1"
        if (Test-Path $practicalTestScript) {
            & $practicalTestScript
        } else {
            throw "PracticalExecutionTests.ps1 not found"
        }
    }
}
...
if (($TestType -eq "All" -or $TestType -eq "Integration")) {
    $integrationTests = @(
        "FinalArchitectureValidation.ps1",
        "PowerShellGetIntegrationTest.ps1",
        "UniversalExecutionTests.ps1"
    )
    
    foreach ($testScript in $integrationTests) {
        $testPath = Join-Path $TestsPath $testScript
        if (Test-Path $testPath) {
            Invoke-TestSuite "Integration: $testScript" {
                & $testPath
            }
        }
    }
...
Write-Host "=" * 40 -ForegroundColor Cyan
Write-Host "Total Test Suites: $($AllTestResults.TotalSuites)" -ForegroundColor White
...
if ($AllTestResults.FailedSuites -gt 0) {
    Write-Host ""
    Write-Host "Failed Test Suites:" -ForegroundColor Red
    foreach ($errorMessage in $AllTestResults.Errors) {
    ...
    Write-Host ""
    Write-Host "OVERALL RESULT: ALL TEST SUITES PASSED" -ForegroundColor Green
    Write-Host "PoShDevModules is fully tested and working correctly!" -ForegroundColor Green
}
