...
"@
    ...
    Context "Metadata Creation" {
        It "Should create metadata directory if it doesn't exist" {
            Save-ModuleMetadata -ModuleName "TestModule" -SourceType "Local" -SourcePath "/test/path" -InstallPath $script:TestInstallPath -LogLevel "Silent"
        ...
        It "Should handle GitHub source type with branch and subpath" {
            Save-ModuleMetadata -ModuleName "TestModule" -SourceType "GitHub" -SourcePath "user/repo" -InstallPath $script:TestInstallPath -Branch "develop" -ModuleSubPath "src/module" -LogLevel "Silent"
    ...
    Context "Error Handling" {
        It "Should not throw error when metadata creation fails" {
            ...
            { Save-ModuleMetadata -ModuleName "TestModule" -SourceType "Local" -SourcePath "/test/path" -InstallPath "/invalid/path" -LogLevel "Silent" } | Should -Not -Throw
        }
    }
}
